...
function EditLecture() {
  ...
  const handleEditLecture = async () => {
    ...
    try {
      ...
      const msg = error?.response?.data?.message || "Error updating lecture"
      if (msg.includes('too large') || msg.includes('FILE_TOO_LARGE')) {
        toast.error(`Video too large. Maximum upload limit is ${MAX_INPUT_MB}MB.`)
      } else {
        toast.error(msg)
      }
    } finally {
  ...
  return (
    <div className='min-h-screen flex items-center justify-center p-6'
      ...
      <style>{`
        ...
        .animate-card { animation: fadeIn 0.4s ease-out; }
      `}</style>

      {/* Card width increased to max-w-2xl (approx 672px) */}
      <div className='w-full max-w-2xl rounded-3xl p-8 space-y-6 animate-card'
        style={{ background: '#ffffff', boxShadow: '0 20px 40px rgba(0,0,0,0.08)', border: '1px solid rgba(0,0,0,0.05)' }}>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div>
            <h2 className='text-2xl font-bold text-[#1E293B]'>Update Lecture</h2>
            <p className='text-sm text-[#64748B]'>Edit lecture details and video</p>
          </div>
        </div>
        ...
        <button className='flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-all'
          ...
          disabled={loading1} onClick={removeLecture}>
          {loading1 ? <ClipLoader size={14} color='#EF4444' /> : '🗑 Remove Lecture'}
        </button>

        <div className='space-y-5'>
          <div>
            <label style={{ display: 'block', fontSize: '0.9rem', fontWeight: 700, color: '#475569', marginBottom: 8 }}>Lecture Title *</label>
            <input type="text" className='w-full p-3 rounded-xl border border-gray-200 text-base outline-none focus:border-[#6C5CE7]'
              value={lectureTitle}
              onChange={(e) => setLectureTitle(e.target.value)} />
          </div>

          <div>
            <label style={{ display: 'block', fontSize: '0.9rem', fontWeight: 700, color: '#475569', marginBottom: 8 }}>
              Video File <span style={{ color: '#94A3B8', fontWeight: 400 }}>(max {MAX_INPUT_MB}MB)</span>
            </label>
            <label className='flex flex-col items-center justify-center w-full p-6 rounded-xl cursor-pointer border-2 border-dashed border-[#E2E8F0] bg-[#F9FAFB] hover:border-[#6C5CE7] transition-all'>
              <span className='text-2xl mb-1'></span>
              <p className='text-sm font-medium text-[#475569]'>{videoFile ? videoFile.name : 'Click to upload or replace video'}</p>
              <input type="file" className='hidden' accept='video/*' onChange={handleVideoSelect} />
            </label>

            {videoError && (
              <div className='mt-3 p-3 rounded-lg bg-red-50 border border-red-200'>
                <p className='text-xs text-red-600 font-bold'>{videoError}</p>
              </div>
            )}
            
            {videoFile && !videoError && (
              <p className='text-xs mt-2 text-green-600'>✓ New file selected: {(videoFile.size / 1024 / 1024).toFixed(1)}MB</p>
            )}
          </div>

          <div className='flex items-center gap-3 p-4 rounded-xl cursor-pointer border border-gray-200 hover:border-[#6C5CE7] transition-all'
            onClick={() => setIsPreviewFree(!isPreviewFree)}>
            <div className={`w-5 h-5 rounded flex items-center justify-center ${isPreviewFree ? 'bg-[#6C5CE7]' : 'bg-gray-200'}`}>
              <span className='text-white text-[10px] font-bold'>✓</span>
            </div>
            <div>
              <p className='text-sm font-bold text-[#1E293B]'>Free Preview {isPreviewFree ? '(ON)' : '(OFF)'}</p>
            </div>
          </div>
        </div>

        <button className='w-full py-3 rounded-xl text-base font-bold text-white transition-all hover:opacity-90'
