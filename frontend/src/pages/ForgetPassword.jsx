...
function ForgetPassword() {
  ...
  return (
    <div className='min-h-screen flex items-center justify-center p-4 relative overflow-hidden'
      ...
      <div className='w-full max-w-md rounded-2xl p-8 relative z-10 fadeInUp'
        style={{ background: 'rgba(255,255,255,0.9)', border: '1px solid rgba(196,181,253,0.5)', boxShadow: '0 8px 40px rgba(124,58,237,0.12)' }}>

        {/* Header */}
        <div className='text-center mb-8'>
        <div className='w-14 h-14 rounded-2xl mx-auto mb-4 flex items-center justify-center text-2xl'
  style={{ background: 'linear-gradient(135deg, #7C3AED, #10B981)', boxShadow: '0 4px 20px rgba(124,58,237,0.3)' }}>
  <RiLockPasswordLine className="text-white text-3xl" />
</div>
          <h1 className='text-2xl font-bold mb-1' style={{
          ...
          {stepLabels.map((label, i) => (
            <React.Fragment key={i}>
              <div className='flex flex-col items-center gap-1'>
                <div className='w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300'
                  style={{
                    background: step > i + 1 ? '#10B981' : step === i + 1 ? 'linear-gradient(135deg, #7C3AED, #10B981)' : 'rgba(124,58,237,0.08)',
                    color: step >= i + 1 ? '#fff' : '#9CA3AF',
                    border: step === i + 1 ? 'none' : '1px solid rgba(124,58,237,0.2)',
                    boxShadow: step === i + 1 ? '0 0 16px rgba(124,58,237,0.35)' : 'none'
                  }}>
                  {step > i + 1 ? '✓' : i + 1}
                </div>
                <span className='text-xs' style={{ color: step === i + 1 ? '#7C3AED' : '#9CA3AF' }}>{label}</span>
              </div>
              {i < 2 && (
                <div className='w-8 h-[2px] mb-4 rounded-full transition-all duration-300'
                  style={{ background: step > i + 1 ? '#10B981' : 'rgba(124,58,237,0.15)' }} />
              )}
            </React.Fragment>
          ))}
        </div>
        ...
        {step === 1 && (
          <div className='space-y-4'>
            ...
            <button onClick={handleSendOtp} disabled={loading}
              className='w-full py-3 rounded-xl font-bold text-base flex items-center justify-center gap-2 transition-all duration-300 disabled:opacity-50 ctaBtn'
              style={{ background: 'linear-gradient(135deg, #7C3AED, #10B981)', color: '#fff', boxShadow: '0 4px 20px rgba(124,58,237,0.3)' }}
        ...
        {step === 2 && (
          <div className='space-y-4'>
            <div>
              <label className='block text-sm font-semibold mb-2' style={{ color: '#374151' }}>Enter OTP</label>
              <input
                type='text'
                placeholder='Enter 4-digit OTP'
                value={otp}
                ...
                onKeyDown={e => e.key === 'Enter' && handleVerifyOtp()}
              />
              <p className='text-xs mt-2' style={{ color: '#9CA3AF' }}>OTP expires in 5 minutes</p>
            </div>
        ...
        {step === 3 && (
          <div className='space-y-4'>
            <div>
              <label className='block text-sm font-semibold mb-2' style={{ color: '#374151' }}>New Password</label>
              <div style={{ position: 'relative' }}>
                <input
                  type={showPassword ? 'text' : 'password'}
                  placeholder='Min 8 characters'
                ...
                />
                <button onClick={() => setShowPassword(p => !p)}
                  style={{ position: 'absolute', right: '14px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#9CA3AF', cursor: 'pointer', fontSize: '16px' }}>
                  {showPassword ? '🙈' : '👁'}
                </button>
              </div>
            </div>
            ...
            <div>
              <label className='block text-sm font-semibold mb-2' style={{ color: '#374151' }}>Confirm Password</label>
              <input
                type={showPassword ? 'text' : 'password'}
                placeholder='Re-enter new password'
                value={confirmPassword}
        ...
        <div className='text-center mt-6'>
          <span className='text-sm' style={{ color: '#9CA3AF' }}>Remember your password? </span>
          <button onClick={() => navigate('/login')}
            style={{ background: 'none', border: 'none', color: '#7C3AED', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>
            Log in
          </button>
        </div>
      </div>
