...
// Category icons with deep rich Purple and elegant Mint/Emerald Green accents
const courseItems = [
  { icon: TbDeviceDesktopAnalytics, label: 'Web Dev',         bg: '#F3E8FF', border: 'rgba(147, 51, 234, 0.2)', iconColor: '#9333EA' },
  { icon: LiaUikit,                 label: 'UI/UX Designing', bg: '#E6F4EA', border: 'rgba(52, 168, 83, 0.2)',  iconColor: '#34A853' },
  { icon: MdAppShortcut,            label: 'App Dev',         bg: '#F3E8FF', border: 'rgba(147, 51, 234, 0.2)', iconColor: '#A855F7' },
  { icon: FaHackerrank,             label: 'Ethical Hacking', bg: '#E6F4EA', border: 'rgba(52, 168, 83, 0.2)',  iconColor: '#10B981' },
  { icon: AiFillOpenAI,             label: 'AI/ML',           bg: '#F3E8FF', border: 'rgba(147, 51, 234, 0.2)', iconColor: '#9333EA' },
  { icon: SiGoogledataproc,         label: 'Data Science',    bg: '#E6F4EA', border: 'rgba(52, 168, 83, 0.2)',  iconColor: '#34A853' },
  { icon: BsClipboardData,          label: 'Data Analytics',  bg: '#F3E8FF', border: 'rgba(147, 51, 234, 0.2)', iconColor: '#A855F7' },
  { icon: SiOpenaigym,              label: 'AI Tools',        bg: '#E6F4EA', border: 'rgba(52, 168, 83, 0.2)',  iconColor: '#10B981' },
];
...
function ExploreCourses() {
  ...
  return (
    // Replaced peach with a smooth, deeper dark-leaning purple and green combination background
    <div 
      className='w-[100vw] min-h-[50vh] lg:h-[50vh] flex flex-col lg:flex-row items-center justify-center gap-4 px-[30px] relative z-10'
      style={{ background: 'linear-gradient(135deg, #F3E8FF 0%, #E8F5E9 100%)' }}
    >
      ...
      <div className='w-[100%] lg:w-[350px] lg:h-[100%] h-[400px] flex flex-col items-start justify-center gap-1 md:px-[40px] px-[20px]'>
        ...
        <span
          className='text-[35px] font-extrabold leading-tight'
          style={{ color: '#0E1B4D' }}
        >
          Explore
        </span>
        ...
        <p className='text-[16px] mt-2 font-medium' style={{ color: '#4B5563', lineHeight: '1.6' }}>
          Explore industry-ready courses designed to boost your skills and career growth.
          Learn smarter with interactive, AI-powered education tailored for the future.
        </p>
        ...
        <button
          onClick={() => navigate("/allcourses")}
          className='px-[24px] py-[12px] rounded-[12px] text-[18px] font-semibold flex gap-2 mt-[40px] cursor-pointer items-center transition-all duration-300 hover:scale-105 active:scale-95'
          style={{
            background: '#6C5CE7',
            color: '#fff',
            border: 'none',
          ...
          onMouseEnter={e => {
            e.currentTarget.style.boxShadow = '0 10px 25px rgba(108, 92, 231, 0.4)';
          }}
          onMouseLeave={e => {
            e.currentTarget.style.boxShadow = '0 6px 20px rgba(108, 92, 231, 0.25)';
          }}
        >
          Explore Courses <SiViaplay className='w-[22px] h-[22px]' style={{ fill: '#fff' }} />
        </button>
      </div>

      {/* Right / Bottom — Grid items with tuned aesthetic shades */}
      <div className='w-[720px] max-w-[90%] lg:h-[300px] md:min-h-[300px] flex items-center justify-center lg:gap-[60px] gap-[50px] flex-wrap mb-[50px] lg:mb-[0px]'>
        {courseItems.map(({ icon: Icon, label, bg, border, iconColor }) => (
          <div
            key={label}
            className='w-[100px] h-[130px] flex flex-col gap-3 text-center items-center cursor-pointer group'
          >
            <div
              className='w-[100px] h-[90px] rounded-2xl flex items-center justify-center transition-all duration-300'
              style={{
                background: bg,
                border: `1px solid ${border}`,
                boxShadow: '0 8px 20px rgba(14, 27, 77, 0.02)',
              }}
              onMouseEnter={e => {
                e.currentTarget.style.boxShadow = '0 12px 24px rgba(108, 92, 231, 0.12)';
                e.currentTarget.style.transform = 'translateY(-4px)';
              }}
              onMouseLeave={e => {
                e.currentTarget.style.boxShadow = '0 8px 20px rgba(14, 27, 77, 0.02)';
                e.currentTarget.style.transform = 'translateY(0)';
              }}
            >
              <Icon className='w-[48px] h-[48px] transition-transform duration-300 group-hover:scale-110' style={{ color: iconColor }} />
            </div>
            <span
              className='text-[13px] font-semibold leading-tight transition-colors duration-300 group-hover:text-[#6C5CE7]'
              style={{ color: '#0E1B4D' }}
            >
              {label}
            </span>
